...
To fully benefit from these protections, ensure `oidc-spa` is the first JavaScript code that runs on your website by following [this setup](/resources/performance-issues.md).  
...
- **(WIP) Preventing `fetch` monkey patching** – `window.fetch` will be frozen to prevent malicious code from overriding it and capturing tokens.  
